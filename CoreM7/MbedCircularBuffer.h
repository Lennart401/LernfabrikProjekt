/* This is a slightly altered version of the CircularBuffer from the mbed os library
 * which supports taking a custom pointers as the buffer array. Therefore, the portentas
 * SDRAM can be used to store that buffer (since it is much larger)
 * 
 * This modification will neither allocate nor free any memory, so that has to be done
 * outside of it by hand. There is no null check of the pointer, so it is necessary to ensure
 * the pointer passed in the constructor is not null.
 */
 ...
 */
#ifndef MBED_CUSTOM_CIRCULARBUFFER_H
#define MBED_CUSTOM_CIRCULARBUFFER_H
